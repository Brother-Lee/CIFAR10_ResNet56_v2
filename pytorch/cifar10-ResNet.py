...
def main():
    epoch=1
    preprocess=transforms.Compose([transforms.RandomHorizontalFlip(),transforms.ToTensor(),Preprocess()])
    trainset=torchvision.datasets.CIFAR10('./data',train=True,download=True,transform=preprocess)
    ...
    epoch=restore(net,optimizer,'ckpt_best/model-247-0.9379.pth')
    max_accuracy=0
    while 1:
